...
| Enable severity-based fail gates on all four security scanners currently non-blocking: Bandit (Security Scan job, every invocation ends in double-pipe true), pip-audit (same job, same pattern), Trivy IaC scan (exit-code: 0 explicitly set for CRITICAL/HIGH/MEDIUM), and DAST/ZAP (fail_action: false, already logged 13.08.2026) -- currently only Tests and Lint (coverage/flake8) can fail the pipeline | no spec | docs-only for initial thresholds; may need tuning once real fail-gate behavior is observed | High | 14.08.2026, verified directly via .github/workflows/deploy-staging-terraform.yml (DevSecOps Governance Part 2 article comparison) -- third consecutive day the same underlying pattern was found in a different layer (13.08 DAST, 14.08 Part 1 deployment gate, 14.08 Part 2 this item); consolidates what would otherwise be four separate near-duplicate backlog entries; cross-reference into security-md-honest-limitations.spec.md alongside the other two findings; matches the exact "quiet casualties" pattern (SAST/SCA present but not enforced, no branch policies, no pre-deployment approvals) described in the series parent post, When Shift-Left Leaves the Back Door Open (Wayne Campbell, 30.06.2026) |
...
| Blue-Green deployment via Azure App Service Deployment Slots (staging slot + swap, optionally canary traffic-splitting) | no spec | tier-gated -- Deployment Slots require Standard/Premium/Isolated App Service tier; this project currently runs B1 (Basic), which does not support slots at all | Low (future consideration, not a current gap) | 15.08.2026 -- evaluated against Blue Green Deployment with Azure App Service Deployment Slots (Sahitya Pai, Capgemini Microsoft Blog, 27.09.2024). Deliberately NOT logged as a backlog gap or Honest Limitations item like the recent DAST/branch-protection/scanner-gate findings -- the project never claimed Blue-Green deployment as a goal, and B1 is an already-documented, deliberate cost/scale tradeoff. Real trigger for revisiting: the CompliGuard fork (multi-tenant SaaS, real paying users) or any scenario where deploy-time downtime becomes user-facing rather than staging-only, at which point the tier upgrade cost becomes justified. |
