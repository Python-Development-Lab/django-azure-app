...
`security/mitre/attack_data.json` — as understood from project documentation, containing entries shaped approximately as:
...
**Honesty note (per this project's own established discipline — see the `ThreatIntelIndicators` lesson in `ioc-reputation-lookup.spec.md`'s Revision Log):** the exact current field names above are *inferred* from consistency with the already-written Evidence-Linked ATT&CK Mapping spec (which assumes pre-existing `technique_id` and `status` fields to extend with a new `evidence` array), not from an independent fresh read of the live file in this session. This is flagged explicitly in Open Question 1 below rather than asserted as confirmed fact.
...
- **REQ-08 (Ubiquitous):** As of this baseline's authoring date, the technique set shall classify 6 techniques as `mitigated`, 8 as `detected`, 1 as `monitored`, and 5 as `gap`, totaling 20 techniques across 9 tactics.
...
- [x] REQ-01/REQ-02 — asserted true based on project documentation describing this exact pattern (HTMX shell + parallel `hx-trigger="load"`); **not freshly re-observed in-browser this session** — treat as "documented, not re-verified"
- [x] REQ-08 counts (6/8/1/5, 20 techniques, 9 tactics) — consistent with figures used throughout this session's project context; **not freshly re-counted against the live file**
- [x] REQ-09 (NMap incident, Zero Trust alerts) — independently corroborated multiple times this session (Attack Path analysis, Sentinel rule descriptions) — **higher confidence than the other checked items above**
- [ ] REQ-03/04/05 — needs a fresh, direct look at `/security/coverage/` output and the current `attack_data.json` file content before this can be marked confirmed rather than assumed
...
1. The exact current JSON schema of `attack_data.json` (section 4) has not been independently re-read in this session — it is inferred from consistency with the Evidence-Linked ATT&CK Mapping spec's assumptions. **Needs verification:** open the actual file and diff against section 4 before treating this baseline as fully confirmed; update this spec if there is any discrepancy (per REQ-01 in `evidence-linked-attack-mapping.spec.md`, which already assumes this schema — a mismatch here would need to be corrected in both specs).
