...
3. Add the new spec to `docs/specs/README.md`'s Index table and Implementation Order section.
4. If the spec depends on or is depended on by another spec, cross-reference both directions (in this spec's Traceability, and by updating the other spec's Traceability if needed).
